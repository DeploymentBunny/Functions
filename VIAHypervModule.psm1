...
#>
    ...
    switch ($VHDClass)
    {
        UEFI {$PartitionNumber = 4}
        BIOS {$PartitionNumber = 2}
        DATA {$PartitionNumber = 1}
        Default {BREAK}
    }
   
    $VHD = Mount-VHD -Path $VHDfile -Passthru -NoDriveLetter
    $MountVHD = New-Item -Path $MountFolder -ItemType Directory -Force
    Add-PartitionAccessPath -DiskNumber $VHD.DiskNumber -PartitionNumber $PartitionNumber -AccessPath $($MountVHD.FullName)
}
